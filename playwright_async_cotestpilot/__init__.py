...
class CheckResult:
    ...
    profile: str
    output_file: Optional[str] = None  # Add output_file field with default None
...
async def check(self, 
    ...
    try:
        ...
        await self.screenshot(path=screenshot_path, type='png')
...
{custom_prompt if custom_prompt else ''}"""
        ...
        if save_to_file:
            ...
            # Load existing results if file exists, or create new list
            existing_results = []
            if os.path.exists(output_file):
                try:
                    ...
                    logger.warning(f"Error reading existing file {output_file}: {str(e)}")
                    existing_results = []
...
async def report(self, output_dir: str = "ai_check_results") -> str:
    """
...
setattr(AsyncPage, "ai_report", report)
...
if not api_key:
    logger.warning("GEMINI_KEY environment variable not set. GPT chat functionality will not work.")
...
def gptchat(prompt, add_time=True):
    ...
    Raises:
        RuntimeError: If GEMINI_KEY environment variable is not set
    """
    if not api_key:
        raise RuntimeError("GEMINI_KEY environment variable must be set to use GPT chat functionality")
...
def chat_vision(prompt, base64_image, add_time=True):
    ...
    try:
        ...
        # print(f"object Response is: {response}")
